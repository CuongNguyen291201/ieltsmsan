...
const MainTopicNode = (props: TopicNodeProps) => {
  ...
  return (
    ...
    //       {isTopicHasChild ? <i className={`fas fa-chevron-${isLoadChild ? 'up' : 'down'} toggle-main`} /> : <i className="toggle-main isStatusDone" />}{!isTopicHasChild && <TopicIcon topicType={topic.type} isMain={true} isTopicOpen={topic.status === STATUS_OPEN} />}
    //       {topic.name}
    //     </div>
